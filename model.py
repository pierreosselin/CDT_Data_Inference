...
class GaussianProcess():
    ...
    def __init__(self, kernel, noise = 1.):
        """
    ...
    def optimize(self, initial_values = [], bounds = None, optimize_noise = False, verbose = 0, random = False):
        ...
        Args:
            ...
            if a None value is given in the list, then the corresponding parameter
            is fixed during optimization. If bounds is not specified, optimize
            according to scipy default optimization procedure.
        """
    ...
    def sequential_prediction(self, t_test, lookahead = 1./24., split = None, optimization = False, random_initialization = False, **kwargs):
        ...
        Args:
            t_test (1d array) : test data
            lookahead (float) : Lookahead for pointwise sequential prediction
            split (int) : number of splits of the data (must divide the number of test data).
            optimization (Bool) : Whether or not optimize sequentially
            random_initialization (Bool) : Whether to initialize the kernel parameters randomly.
